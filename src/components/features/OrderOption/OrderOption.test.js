...
describe('Component OrderOption', () => {
  it('should render without crashing', () => {
    const component = shallow(<OrderOption type={'number'} name={'test'} />);
  ...
  it('should render with props name', () => {
    ...
    switch (type) {
      ...
      case 'icons': {
        it('contains div with icons', () => {
          ...
          const emptyDivIcon = mainDiv.find('.icon').find('Icon[name="times-circle"]').length;
          expect(emptyDivIcon).toBe(1);

          const divIcons = mainDiv.findWhere((n) => n.key());
      ...
      case 'checkboxes': {
        it('contains div with inputs', () => {
          ...
          const inputs = mainDiv.find('label').find('input[type="checkbox"]');
